...
# Ensure UTF-8 streams for stdout/stderr
...
SAFE_PLACEHOLDER_VALUES = {
    ...
    "example@example.com",
    "security@open-jarvis.org",
    "http://127.0.0.1:8888/callback",
    "false",
    "true",
}

SAFE_EMAILS = {"example@example.com", "test@example.com", "security@open-jarvis.org"}
...
def _is_safe_placeholder(value: str) -> bool:
    ...
    if cleaned in SAFE_PLACEHOLDER_VALUES or cleaned.isdigit():
        return True
    return cleaned.startswith("your_") or cleaned.startswith("your-") or cleaned.endswith("_here") or cleaned.startswith("example")
...
def _scan_line(path: Path, rel: Path, line_number: int, line: str) -> list[Finding]:
    ...
    for email in EMAIL_RE.findall(line):
        if email.lower() not in SAFE_EMAILS and "example." not in email.lower() and "open-jarvis.org" not in email.lower():
    ...
    if WINDOWS_USER_PATH_RE.search(line) and "C:\\Path\\To\\Your\\File" not in line and "C:\\Path\\To" not in line:
...
def _looks_documented_or_code_reference(rel_name: str, lower_line: str) -> bool:
    if rel_name.startswith("tests/") or rel_name.endswith((".md", ".py", ".yml", ".yaml", ".txt", ".example")):
        if any(token in lower_line for token in ("placeholder", "example", "missing", "mask", "pattern", "never commit", "secret_keys", "authorization", "bearer")):
            return True
        if any(token in lower_line for token in ("os.getenv", "getenv", "client_secret", "spotify_client_secret", "headers")):
            return True
    return False
...
def scan_file(path: Path, root: Path = ROOT) -> list[Finding]:
    rel = _relative(path, root)
    if BLOCKED_FILE_RE.search(rel.as_posix()) and rel.name not in (".env.example", ".gitignore", ".env"):

...
class TeluguNativeConversationSystemTests(unittest.TestCase):
    ...
    def tearDown(self):
        # Reset preferences to default english after each test run
        set_preference("preferred_language", "english")
        set_preference("learning_command", None)
        set_preference("learned_commands", {})
    ...
    def test_enforced_language_mode_persistence(self):
        ...
        format_telugu_response("Hello", "open chrome")
        self.assertEqual(get_preference("language_mode"), "telugu")
        self.assertEqual(get_preference("preferred_language"), "telugu")
    ...
    def test_voice_routing_rules(self):
        import asyncio
        from JARVIS.core.voice.ses_motoru import _speak_async, VoiceEngine

        with patch.object(VoiceEngine, "_speak_async_direct", new_callable=unittest.mock.AsyncMock) as mock_direct:
            asyncio.run(_speak_async("సమయం ఎంత"))
            mock_direct.assert_called_with("సమయం ఎంత", "te-IN-MohanNeural")

            mock_direct.reset_mock()
            asyncio.run(_speak_async("Hello, how are you?"))
            mock_direct.assert_called_with("Hello, how are you?", "en-US-AriaNeural")

    def test_no_false_positive_intercept(self):
        from JARVIS.runtime.self_healing import HIGH, SelfHealingEngine

        # Instantiate the singleton and set pending_repairs directly
        engine = SelfHealingEngine()
        engine.pending_repairs = {"some_issue": {"id": "some_issue", "name": "Some issue", "risk": HIGH, "action": "Some action"}}

        with patch("JARVIS.core.automation.komutlar.speak") as speak_mock:
            # We mock route_local_intent to return a valid dummy action so we don't depend on LLM/Groq router
            with patch("JARVIS.core.automation.local_intent_router.route_local_intent", return_value={"action": "talk", "response": "Hello"}):

...
import JARVIS.core.system.utils.kontrol as kontrol
from JARVIS.core.security.jarvis_admin import render_health_report


class HealthCliTests(unittest.TestCase):
    def test_should_pause_skips_ci_and_no_pause_flag(self):
        self.assertFalse(kontrol.should_pause(["JARVIS.core.system.utils.kontrol.py", "--no-pause"], {}))
        self.assertFalse(kontrol.should_pause(["JARVIS.core.system.utils.kontrol.py"], {"CI": "true"}))
        self.assertTrue(kontrol.should_pause(["JARVIS.core.system.utils.kontrol.py"], {}))

    def test_project_audit_check_reports_findings(self):
        with patch("JARVIS.core.system.utils.kontrol.read_project_files", return_value={"large.py": "\n".join(["x = 1"] * 501)}):
    ...
    def test_main_does_not_prompt_when_no_pause_flag_is_used(self):
        with (
            ...
            result = kontrol.main(["JARVIS.core.system.utils.kontrol.py", "--no-pause"])
    ...
    def test_main_enables_explicit_provider_probe_flag(self):
        with (
            ...
            result = kontrol.main(["JARVIS.core.system.utils.kontrol.py", "--probe-providers", "--no-pause"])

...
from JARVIS.core.automation.action_dispatcher import execute_action
from JARVIS.core.automation.domains.memory_actions import handle_memory_action
from JARVIS.core.automation.domains.runtime_actions import env_float, handle_runtime_action
...
class ActionDispatcherTest(unittest.TestCase):
    def test_execute_action_routes_and_speaks_response(self):
        ...
        with patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True) as single_mock:
    ...
    def test_multi_action_delay_is_configurable(self):
        ...
        with (
            ...
            patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True),
            patch("JARVIS.core.automation.action_dispatcher.time.sleep") as sleep_mock,
        ):
    ...
    def test_execute_action_auto_heals_string_payload(self):
        spoken = []
        context = {
            "speak": spoken.append,
            "logger": DummyLogger(),
        ...
        with patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True) as single_mock:
            result = execute_action("hello sir", context)
        self.assertTrue(result)
        self.assertEqual(spoken[0], "hello sir")
        single_mock.assert_called_once_with("talk", {}, context)

    def test_execute_action_auto_heals_empty_action(self):
        spoken = []
        context = {
            "speak": spoken.append,
            "logger": DummyLogger(),
        ...
        with patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True) as single_mock:
            result = execute_action({"action": "", "response": "healed"}, context)
        self.assertTrue(result)
        self.assertEqual(spoken[0], "healed")
        single_mock.assert_called_once_with("talk", {}, context)

    def test_execute_action_auto_heals_invalid_response_type(self):
        spoken = []
        context = {
            "speak": spoken.append,
            "logger": DummyLogger(),
        ...
        with patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True) as single_mock:
            result = execute_action({"action": "talk", "response": 123}, context)
        self.assertTrue(result)
        self.assertEqual(spoken[0], "123")

    def test_execute_action_auto_heals_empty_actions_list(self):
        spoken = []
        context = {
            "speak": spoken.append,
            "logger": DummyLogger(),
        ...
        with patch("JARVIS.core.automation.action_dispatcher.execute_single_action", return_value=True) as single_mock:
            result = execute_action({"actions": [], "response": "healed empty actions"}, context)
        self.assertTrue(result)
        self.assertEqual(spoken[0], "healed empty actions")
        single_mock.assert_called_once_with("talk", {}, context)
    ...
    def test_runtime_helpers_handle_invalid_env_and_launch_known_app(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.os.path.exists", return_value=True),
            patch("JARVIS.core.automation.domains.runtime_actions.launch_process") as launch_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.record_runtime_event") as event_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
    ...
    def test_runtime_handler_launches_app_from_system_path_fallback(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.os.path.exists", return_value=False),
            patch("JARVIS.core.automation.domains.runtime_actions.launch_process") as launch_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.record_runtime_event") as event_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
            patch.dict("os.environ", {"JARVIS_APP_LAUNCH_DELAY": "0"}),
    ...
    def test_runtime_handler_blocks_destructive_actions_without_permission(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.run_command") as run_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.record_runtime_event") as event_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.is_destructive_action_allowed", return_value=False),
        ):
    ...
    def test_runtime_handler_reports_missing_battery(self):
        ...
        with patch("JARVIS.core.automation.domains.runtime_actions.psutil.sensors_battery", return_value=None):
    ...
    def test_runtime_handler_reads_empty_clipboard(self):
        ...
        with patch("JARVIS.core.automation.domains.runtime_actions.pyperclip.paste", return_value=""):
    ...
    def test_runtime_handler_truncates_long_clipboard_text(self):
        ...
        with patch("JARVIS.core.automation.domains.runtime_actions.pyperclip.paste", return_value="x" * 900):
            result = handle_runtime_action("read_clipboard", {}, context)
    ...
    def test_runtime_handler_summarizes_clipboard_with_context_callback(self):
        ...
        with patch("JARVIS.core.automation.domains.runtime_actions.pyperclip.paste", return_value="long text"):
    ...
    def test_runtime_handler_summarize_clipboard_empty_and_local_fallback_summary(self):
        ...
        with patch("JARVIS.core.automation.domains.runtime_actions.pyperclip.paste", side_effect=["", "important text"]):
    ...
    def test_runtime_handler_keyboard_and_mouse_actions(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.typewrite") as typewrite_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.hotkey") as hotkey_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.click") as click_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.scroll") as scroll_mock,
        ):
    ...
    def test_runtime_handler_keyboard_mouse_and_scroll_edge_paths(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.typewrite") as typewrite_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.press") as press_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.doubleClick") as double_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.scroll") as scroll_mock,
        ):
    ...
    def test_runtime_handler_covers_system_metrics_and_desktop_controls(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.psutil.sensors_battery", return_value=Battery()),
            patch("JARVIS.core.automation.domains.runtime_actions.psutil.virtual_memory", return_value=Memory()),
            patch("JARVIS.core.automation.domains.runtime_actions.psutil.cpu_percent", return_value=12.5),
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.hotkey") as hotkey_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.run_command") as run_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.is_destructive_action_allowed", return_value=True),
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
    ...
    def test_runtime_handler_screenshot_and_clipboard_error_paths_are_safe(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.os.makedirs") as makedirs_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.pyautogui.screenshot", return_value=Screenshot()),
            patch("JARVIS.core.automation.domains.runtime_actions.launch_process") as launch_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep"),
            patch("JARVIS.core.automation.domains.runtime_actions.pyperclip.paste", side_effect=RuntimeError("clipboard locked")),
    ...
    def test_runtime_handler_launch_app_failure_and_unknown_action(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.os.path.exists", return_value=False),
            patch("JARVIS.core.automation.domains.runtime_actions.launch_process", side_effect=OSError("missing")),
        ):
    ...
    def test_runtime_handler_power_actions_when_explicitly_allowed(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.is_destructive_action_allowed", return_value=True),
            patch("JARVIS.core.automation.domains.runtime_actions.run_command") as run_mock,
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep") as sleep_mock,
    ...
    def test_runtime_handler_sleep_honors_configured_delay(self):
        ...
        with (
            patch("JARVIS.core.automation.domains.runtime_actions.is_destructive_action_allowed", return_value=True),
            patch("JARVIS.core.automation.domains.runtime_actions.run_command"),
            patch("JARVIS.core.automation.domains.runtime_actions.time.sleep") as sleep_mock,
    ...
    def test_memory_handler_reads_notes(self):
        ...
        with patch("JARVIS.core.automation.domains.memory_actions.get_notes", return_value=[{"text": "alpha"}, {"text": "beta"}]):
    ...
    def test_memory_handler_reports_stats(self):
        ...
        with patch("JARVIS.core.automation.domains.memory_actions.get_stats") as stats_mock:
    ...
    def test_memory_handler_covers_habits_health_summary_prune_and_empty_note(self):
        ...
        with (
            ...
            patch("JARVIS.core.automation.domains.memory_actions.get_notes", return_value=[]),
        ):
    ...
    def test_memory_handler_respects_privacy_mode_for_note_writes(self):
        ...
        with patch("JARVIS.core.automation.domains.memory_actions.add_note") as add_note_mock:

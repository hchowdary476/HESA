...
from JARVIS.runtime.orchestrator import handle_runtime_command, should_return_to_standby
...
class RuntimeOrchestratorTest(unittest.TestCase):
    ...
    def test_should_return_to_standby_respects_timeout(self):
        with patch("JARVIS.runtime.orchestrator.time.time", return_value=12.0):
            self.assertFalse(should_return_to_standby(10.0, 5))
        with patch("JARVIS.runtime.orchestrator.time.time", return_value=1.0):

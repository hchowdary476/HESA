...
class MemoryPrivacyControlsTests(unittest.TestCase):
    ...
    def test_config_privacy_disables_normal_memory_note_writes(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                patch("JARVIS.core.memory.memory_notes.load_memory", return_value=memory) as load_memory,
                patch("JARVIS.core.memory.memory_notes.save_memory") as save_memory,
            ):
    ...
    def test_privacy_mode_suppresses_personalization_context_and_reads(self):
        with TemporaryDirectory() as tmp:
            ...
            with (
                patch("JARVIS.core.memory.load_memory") as context_memory,
                patch("JARVIS.core.memory.memory_habits.load_memory") as habits_memory,
                patch("JARVIS.core.memory.memory_preferences.load_memory") as preference_memory,
                patch("JARVIS.core.memory.memory_notes.load_memory") as notes_memory,
            ):
    ...
    def test_groq_context_does_not_include_persisted_memory_during_privacy_mode(self):
        ...
        with (
            ...
            patch.dict("os.environ", {"JARVIS_PRIVACY_MODE": "true", "GROQ_API_KEY": "dummy-key"}),
            patch("JARVIS.config.manager.ConfigManager._read_file_payload", return_value={}),
            patch("JARVIS.core.memory.load_memory", return_value=memory),
            patch("JARVIS.core.memory.get_top_habits", return_value=[("private command", 9)]),
        ):

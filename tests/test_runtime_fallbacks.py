...
from JARVIS.runtime import command_listener, readiness
...
class RuntimeFallbackTests(unittest.TestCase):
    def test_command_listener_reports_missing_microphone_without_raising(self):
        ...
        with (
            patch("JARVIS.core.voice.microphone.SoundDeviceMicrophone", side_effect=OSError("no input device")),
            patch("JARVIS.runtime.command_listener.sr.Microphone", side_effect=OSError("no input device")),
        ):
    ...
    def test_emit_startup_readiness_logs_optional_missing_services(self):
        ...
        self.assertIn("[WARN] Groq API key not found. Running local backup routing, sir.", logs)
    ...
    def test_emit_startup_readiness_uses_resolved_config_when_env_not_injected(self):
        ...
        with patch("JARVIS.runtime.readiness.resolved_env", return_value=resolved):
            report = readiness.emit_startup_readiness(
                send_log=logs.append,

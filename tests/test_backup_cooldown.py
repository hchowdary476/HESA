...
from JARVIS.services.supervisor import BACKUP_METRICS, calculate_sha256, create_backup


class TestBackupCooldown(unittest.TestCase):
    def setUp(self):
        # Save original metrics and reset
        self.original_metrics = {"config": dict(BACKUP_METRICS["config"]), "memory": dict(BACKUP_METRICS["memory"])}

        # Reset state
        for key in ["config", "memory"]:
            BACKUP_METRICS[key] = {"last_backup_time": 0.0, "backup_count": 0, "skipped_backups": 0, "last_sha256": "", "last_mtime": 0.0}

...
def _log_all_input_devices() -> None:
    ...
    try:
        ...
        for idx, ha in enumerate(host_apis):
            logger.info("[VOICE]   Host API %d: %s (default input device: %s)",
                        idx, ha['name'], ha.get('default_input_device', 'none'))
        for idx, dev in enumerate(devices):
            if dev['max_input_channels'] > 0:
                ha_name = host_apis[dev['hostapi']]['name'] if dev['hostapi'] < len(host_apis) else '?'
                logger.info("[VOICE]   Input Device %d: '%s' [%s] channels=%d samplerate=%.0f",
                            idx, dev['name'], ha_name, dev['max_input_channels'],
...
def _resolve_best_input_device(requested_device: Optional[int] = None) -> Tuple[Optional[int], str]:
    ...
    if requested_device is not None:
        try:
            ...
            logger.info("[VOICE] Using explicitly requested device %d '%s' [%s]",
                        requested_device, dev_info['name'], ha_info['name'])
            return (requested_device, ha_info['name'])
        except Exception as exc:
            logger.warning("[VOICE] Requested device %d is invalid (%s), falling back to auto-select",
            ...
            for preferred_name in _WINDOWS_HOSTAPI_PREFERENCE:
                for ha_idx, ha in enumerate(host_apis):
                    ...
                    if default_dev is not None and default_dev >= 0:
                        ...
                        if dev['max_input_channels'] > 0:
                            logger.info("[VOICE] Auto-selected device %d '%s' on preferred host API '%s'",
                                        default_dev, dev['name'], ha['name'])
...
def _try_open_input_stream(*, samplerate, channels, dtype, blocksize, device, callback, **extra_kwargs):
    ...
    try:
        ...
        logger.info("[VOICE] InputStream opened successfully on device=%s '%s'",
                    device, dev_info.get('name', 'system-default'))
        return stream
    except Exception as exc:
    ...
    if device is not None:
        try:
            ...
            if native_rate and native_rate != samplerate:
                ...
                stream = sd.InputStream(
                    samplerate=native_rate, channels=channels, dtype=dtype,
                    blocksize=blocksize, device=device, callback=callback,
                    **extra_kwargs,
                )
                stream.start()
                logger.info("[VOICE] InputStream opened on device=%s '%s' at native %.0f Hz",
                            device, dev_info.get('name', '?'), native_rate)
                return stream
        except Exception as exc:
            ...
            for rate in rates_to_try:
                try:
                    stream = sd.InputStream(
                        ...
                        blocksize=blocksize, device=fallback_dev, callback=callback,
                        **extra_kwargs,
                    )
                    stream.start()
                    logger.info("[VOICE] InputStream opened on FALLBACK device %d '%s' [%s] at %.0f Hz",
                                fallback_dev, devices_list[fallback_dev]['name'], ha['name'], rate)
                    return stream
                except Exception as fb_err:
                    errors[f"device={fallback_dev} ({ha['name']}, {rate}Hz)"] = str(fb_err)
                    logger.warning("[VOICE] Fallback device %d [%s] at %.0f Hz failed: %s",
                                   fallback_dev, ha['name'], rate, fb_err)
    ...
    raise sd.PortAudioError(
        f"All audio input devices failed. Tried: {err_summary}"
    )

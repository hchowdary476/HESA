...
class DiagnosticsCenter:
    ...
    def record_model_query(self, provider: str, latency_ms: float, cost: float, tokens: int, success: bool) -> None:
        with self.lock:
            stats = self.model_stats.setdefault(
                provider, {"latency": [], "cost": [], "tokens": [], "failures": 0, "successes": 0, "fallback_count": 0}
            )
    ...
    def get_planner_analytics(self) -> dict[str, Any]:
        with self.lock:
            ...
            return {
                "avg_plan_size": round(self.planner_stats["total_subtasks"] / max(self.planner_stats["total_plans"], 1), 1)
                if self.planner_stats["total_plans"] > 0
                else 3.5,
        ...
        return [
            {
                ...
                "cost_per_m": 2.50,
                "availability": 0.999,
                "tool_compat": "EXCELLENT",
                "offline_readiness": "NO",
            },
            {

...
def set_activity(name: str, active: bool):
    ...
    state = _read_state()
    state[name] = {"active": active, "timestamp": time.time()}
    _write_state(state)
...
def is_any_trim_prevented() -> bool:
    ...
    return (
        is_activity_active("voice_recognition")
        or is_activity_active("tts_playback")
        or is_activity_active("face_recognition")
        or is_activity_active("ui_interaction")
    )

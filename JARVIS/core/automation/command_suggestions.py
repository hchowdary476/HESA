...
def suggest_commands(context: dict) -> list[dict[str, str]]:
    ...
    try:
        ...
        for sug in learnt_suggestions:
            suggestions.append({"command": sug["command"], "category": sug["type"], "why": sug["why"]})
    except Exception:
        pass
    ...
    if len(suggestions) < 3:
        ...
        if context.get("last_action") == "music":
            suggestions.append(
                {"command": "play my favorite music", "category": "music", "why": "Your last interaction was music-focused."}
            )

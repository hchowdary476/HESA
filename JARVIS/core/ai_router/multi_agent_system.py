...
class Agent:
    ...
    def __init__(
        self, name: str, description: str, system_prompt: str, capabilities: list[str], tools: list[Callable], preferred_model: str
    ) -> None:
        self.name = name
    ...
    def add_task(self, task_id: str, prompt: str, callback: Callable | None = None) -> None:
        ...
        self.task_queue.put({"id": task_id, "prompt": prompt, "callback": callback, "enqueued_at": time.time()})
...
class AgentManager:
    ...
    def _setup_agents(self) -> None:
        # Mock tools mapping
        def tool_code_gen(p):
            return "Code generated and verified successfully."

        def tool_search_web(p):
            return "Search yields 4 relevant documentation links."

        def tool_security_audit(p):
            return "Cyber logs audit complete: 0 vulnerabilities found."

        def tool_os_cmd(p):
            return "Application started, window focused."

        def tool_train_ml(p):
            return "ML model trained. R2 score: 0.94."

        def tool_learn_habits(p):
            return "User habits analyzed. Suggestions updated."
        ...
        agent_configs = [
            (
                ...
                "Generate clean code",
                ["coding", "formatting", "linting", "pytest"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Research Agent",
                "Fetches documentation/papers",
                "Perform system research",
                ["research", "literature", "arxiv", "web_search"],
                [tool_search_web],
                "Gemini 1.5 Pro",
            ),
            (
                "AI & ML Agent",
                "Trains models, dataset indexing",
                "Run training models",
                ["train", "dataset", "benchmarks", "sweeps"],
                [tool_train_ml],
                "Gemini 1.5 Pro",
            ),
            (
                "Cyber Security Agent",
                "Audits connections and CVEs",
                "Audit logs",
                ["security", "port_scan", "log_audit", "cve"],
                [tool_security_audit],
                "ChatGPT 4o",
            ),
            (
                "Developer Agent",
                "Handles build, test and refactor",
                "Build and compile",
                ["build", "compile", "refactor", "deploy"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Windows System Agent",
                "Controls registry, services and CLI",
                "Execute Windows commands",
                ["system", "shutdown", "restart", "sleep", "lock", "process"],
                [tool_os_cmd],
                "Ollama (Llama 3)",
            ),
            (
                "Automation Agent",
                "Controls keyboard, mouse, triggers",
                "Trigger automation scripts",
                ["keyboard", "mouse", "spotify", "play"],
                [tool_os_cmd],
                "LM Studio (Mistral)",
            ),
            (
                "Memory Agent",
                "Maintains local context and notes",
                "Retrieve context from knowledge graph",
                ["memory", "habit", "routine", "learn", "notes"],
                [tool_learn_habits],
                "Ollama (Llama 3)",
            ),
            ...
            (
                "Architect Agent",
                "Analyse requirements and select tech stack",
                "Design software architecture specs",
                ["architecture", "spec", "design", "requirements"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Frontend Agent",
                "Generates user interface layouts",
                "Generate React, Next.js, and HTML frontend assets",
                ["frontend", "ui", "react", "next", "html"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Backend Agent",
                "Generates REST APIs and schemas",
                "Generate FastAPI, Django, Flask, database schemas, auth modules",
                ["backend", "api", "fastapi", "flask", "django", "spring"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Mobile Agent",
                "Generates Flutter and Android mobile client code",
                "Generate mobile flutter widgets, sign configurations, gradle rules",
                ["mobile", "android", "flutter", "apk"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Testing Agent",
                "Generates test suites and run validations",
                "Generate pytest unit tests, vitest specs, check coverage",
                ["testing", "unit_test", "integration_test", "coverage"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Debugger Agent",
                "Analyzes stack traces and compiler/runtime errors",
                "Debug errors, compile and trace fixes",
                ["debugger", "debug", "error_fix"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                "Documentation Agent",
                "Generates README, API docs, and architecture diagrams",
                "Generate README, API docs, architecture diagrams, deployment files",
                ["documentation", "readme", "api_docs", "changelog"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
            (
                ...
                "Generate Docker files, docker-compose orchestration, github actions",
                ["devops", "docker", "ci_cd", "github_actions"],
                [tool_code_gen],
                "Claude 3.5 Sonnet",
            ),
        ]
